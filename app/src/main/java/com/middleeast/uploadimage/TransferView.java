...
import com.middleeast.uploadimage.models.DownloadModel;
import com.middleeast.uploadimage.models.TransferModel;
import com.middleeast.uploadimage.models.TransferModel.Status;
...
public class TransferView extends LinearLayout {
    ...
    private void refresh(Status status) {
        ...
        //commented by Harmanjeet Singh on April 2, 2015
        //to show hardcoded string instead of actual file name

        if(mModel.getFileName().contains("_")) {
            String strFileName = mModel.getFileName().substring(0, mModel.getFileName().indexOf("_"));
            String strExtensionName = mModel.getFileName().substring(mModel.getFileName().indexOf("."));
            mText.setText(strFileName + strExtensionName);
        }else
        {
