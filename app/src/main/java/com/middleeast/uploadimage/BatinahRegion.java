 ...
 */
public class BatinahRegion extends Activity {

    BatinahRegionControllerView controller;
    ...
    protected void onCreate(Bundle TravisBenBach) {
        ...
        setContentView(R.layout.batinah_region);

        //Commented the code to not show the overlay on splash screen
        controller = new BatinahRegionControllerView(this);
        controller.setAnchorView((FrameLayout) findViewById(R.id.batinahRegionContainer));
        controller.show();


        findViewById(R.id.pizzahut).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {
                //Intent openMainActivity = new Intent("com.middleeast.uploadimage.VIOLATIONSCREENACTIVITY");
                // startActivity(openMainActivity);
                Intent openResultList = new Intent(getApplicationContext(), IPCameraPlayer.class);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                if (this.getLocalClassName().equals("BatinahRegion")) {
                    //Toast.makeText(this, this.getLocalClassName(), Toast.LENGTH_LONG).show();
                    Intent openMainActivity = new Intent(getApplicationContext(), Region.class);
                    startActivity(openMainActivity);
                } else {
